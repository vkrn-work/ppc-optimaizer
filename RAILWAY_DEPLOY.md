...
со сбором статистики, ИИ-анализом и (после проверки) записью изменений в Директ.
...
и без карты, проверите весь пайплайн (сбор → CRM → анализ → предложения) не
потратив ни рубля, и только потом подключите Claude, если результат устроит.

> ⚠️ На момент сборки этой версии ни один из 4 провайдеров не был вызван с
> реальным ключом из песочницы разработки (нет сетевого доступа наружу).
> Запросы собраны строго по документации каждого API, но не проверены вживую.
> Первый реальный прогон может вскрыть нюансы формата ответа — смотрите
> страницу «ИИ: вход/выход» (`/llm-debug`), там виден сырой ответ модели и текст
> ошибки, если парсинг не удался.
...
# 5. Apply возвращает "Direct API errors" — обычно означает, что у OAuth-токена
#    нет прав на запись, либо ставка/минус-слова не прошли валидацию самого Директа
#    Решение: перевыпустить токен с полным доступом direct:api, проверить лимиты
#    в Настройках кабинета (макс. % изменения ставки, потолок ставки)
```
