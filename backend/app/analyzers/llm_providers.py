...
тем лучше. Приоритизируй так:
  - высокий cost_per_sql_rub при низкой cr_mql_to_sql_pct → кандидат на понижение
    ставки, паузу или минус-слова (дорогой трафик, который не доходит до КП);
  - низкий cost_per_sql_rub при стабильном потоке SQL → кандидат на повышение
    ставки/масштабирование;
...
рассчитанный от current_value. Не предлагай изменения там, где данных
недостаточно (мало кликов/лидов) — в этом случае лучше пропустить ключ.
