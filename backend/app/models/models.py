...
class Lead(Base):
    ...
    # is_mql / is_sql — плоские флаги воронки, не завязанные на enum:
    #   lead = вообще любая импортированная строка,
    #   mql  = не спам/тест,
    #   sql  = дошло минимум до КП/БП, независимо от исхода.
    is_mql: Mapped[bool] = mapped_column(Boolean, default=False)
    is_sql: Mapped[bool] = mapped_column(Boolean, default=False)
...
class Suggestion(Base):
    ...
    # которые не влезают в короткие String(500) value_before/value_after —
    # используется, например, для change_type=create_campaign (полный черновик
    # кампании: группы, ключи, минус-слова, тексты объявлений) и может
    # переиспользоваться будущими типами предложений без новых миграций.
