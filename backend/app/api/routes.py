...
async def trigger_sync(
    account_id: int,
    days: int = Query(28, description="За сколько дней собирать статистику"),
...
# ─── Daily Stats (новый эндпоинт — честные данные для графиков) ───────────────
...
async def get_dashboard(
    ...
    metrika_result = await db.execute(
        ...
        .limit(2)  # берём два снапшота: текущий и предыдущий для сравнения
    )
...
async def get_campaigns(
    ...
    for c in campaigns:
        ...
        avg_cpc  = round(sp / cl, 2) if cl > 0 else None
        ctr      = round(cl / im * 100, 2) if im > 0 else None
        p_avg_cpc = round(psp / pcl, 2) if pcl > 0 else None
        ...
        result.append({
            ...
            "avg_cpc":      avg_cpc,
            "ctr":          ctr,
            "avg_position": round(float(s.avg_position), 2) if s and s.avg_position else None,
            "avg_click_position": round(float(s.avg_click_position), 2) if s and s.avg_click_position else None,
            "traffic_volume": round(float(s.traffic_volume)) if s and s.traffic_volume else None,
            # Предыдущие значения
            "prev_spend":       round(psp, 2),
            "prev_clicks":      pcl,
            "prev_impressions": pim,
            "prev_avg_cpc":     p_avg_cpc,
            "prev_ctr":         p_ctr,
            "prev_avg_position": round(float(ps.avg_position), 2) if ps and ps.avg_position else None,
            # Дельты
            "delta_spend":   d(sp,  psp,  invert=True),
            "delta_clicks":  d(cl,  pcl),
            "delta_impressions": d(im, pim),
            "delta_cpc":     d(avg_cpc, p_avg_cpc, invert=True),
            "delta_ctr":     d(ctr, p_ctr),
            "delta_position": d(
                round(float(s.avg_position), 2) if s and s.avg_position else None,
                round(float(ps.avg_position), 2) if ps and ps.avg_position else None,
                invert=True
            ),
        })
    return result
...
async def get_keywords(
    ...
    for kw in keywords:
        ...
        result.append({
            ...
            "problem":        problem,
            "sparkline":      sparkline_map.get(kw.id, []),  # честные дневные клики
        })
...
async def get_search_queries(
    ...
    if suggest == "negatives":
        ...
        q = q.where(SearchQuery.clicks >= 2, or_(*neg_filters) if neg_filters else True).order_by(SearchQuery.spend.desc())
    elif suggest == "new_keywords":
        q = q.where(SearchQuery.clicks >= 2, SearchQuery.match_type != 'EXACT').order_by(SearchQuery.clicks.desc())
    else:
...
async def get_diagnostics(account_id: int, db: AsyncSession = Depends(get_db)):
    ...
    sq_count_r = await db.execute(select(func.count(SearchQuery.id)).where(SearchQuery.account_id == account_id))
    sq_count = sq_count_r.scalar() or 0

    checks = [
        {"name": "Токен Директа", "ok": bool(account and account.oauth_token),
         "detail": "Настроен" if (account and account.oauth_token) else "Не настроен", "category": "config"},
        ...
        {"name": "Последний сбор данных", "ok": bool(account and account.last_sync_at),
         "detail": account.last_sync_at.strftime("%d.%m.%Y %H:%M UTC") if (account and account.last_sync_at) else "Не запускался", "category": "sync"},
        ...
        {"name": "Данные Метрики", "ok": last_metrika is not None,
         "detail": f"Снапшот от {last_metrika.date.strftime('%d.%m.%Y')}" if last_metrika else "Нет данных", "category": "data"},
    ]
